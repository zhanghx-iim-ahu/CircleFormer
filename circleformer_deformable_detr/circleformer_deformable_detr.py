# ------------------------------------------------------------------------
# CircleFormer Deformable DETR
# Copyright (c) 2022 Hengxu Zhang. All Rights Reserved.
...
from .segmentation import (DETRsegm, PostProcessPanoptic, PostProcessSegm,
                           dice_loss, sigmoid_focal_loss, PostProcessInstance)
from .deformable_transformer import build_deforamble_transformer
import copy

from ..circleformer_DETR.segmentation import instance_dice_loss
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        if not two_stage:
            if not use_dab:
                ...
                # self.refpoint_embed.weight.data[:, 2:].uniform_(-0.1, 0.1)
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        boxes = box_ops.box_xyr_to_xyxy(out_bbox)
...
class CirclePostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        # results = [{'scores': s, 'labels': l, 'boxes': b} for s, l, b in zip(scores, labels, boxes)]
        results = [{'scores': s, 'labels': l, 'boxes': b, "true_boxes": t} for s, l, b, t in
                   zip(scores, labels, boxes, true_boxes)]
...
def build_circleformer_deformable_detr(args):
    ...
    postprocessors = {'circle': CirclePostProcess()}
    if args.masks:
