    ...
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, use_dab=False, d_model=256,
        ...
        if use_dab:
            ...
            if self.no_sine_embed:
                ...
                self.ref_point_head = MLP(384, d_model, d_model, 2)
        self.high_dim_query_update = high_dim_query_update
        if high_dim_query_update:
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if reference_points.shape[-1] == 4:
                ...
                if len(reference_points.shape) == 2:
                    reference_points_input = reference_points[None, :, None] \
                                             * torch.cat(
                        [src_valid_ratios, torch.mean(src_valid_ratios, dim=2, keepdim=True)],  -1)[:, None]
                else:
                    reference_points_input = reference_points[:, :, None] \
                                             * torch.cat(
                        [src_valid_ratios, torch.mean(src_valid_ratios, dim=2, keepdim=True)], -1)[:, None]
